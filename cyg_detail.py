...
def good(url, expireTime, status, dealTime):
    ...
    searchinfo = {"url": url}
    print(url)
    cursor.execute(searchsql, searchinfo)
    data = cursor.fetchone()
    insertSql = "insert ignore into goods (name,sex,school,level,price,equip_score,mount,special_equip,skill_score," \
                "practice_score,gem_score,status,sell_expire_time,publicity_expire_time,url,max_hp,max_mp," \
                "server_info,deal_time) " \
                "values " \
                "(%(name)s,%(sex)s,%(menpai)s,%(level)s,%(price)s,%(equipScore)s,%(zuoqi)s,%(chonglou)s,%(xinfa)s," \
                "%(xiulian)s,%(gemXiuLian)s," \
                "%(status)s,%(sellTime)s,%(publicityTime)s,%(url)s,%(maxHp)s,%(maxMp)s,%(qufu)s,%(dealTime)s);"

    updateSql = "update goods set price = %(price)s,version = %(version)s, status = %(status)s where id = %(id)s"
    ...
    # 说明是重复数据
    if data is not None and data[4] == float(goodsSoup.find('span', class_='ui-money-color').string.strip("￥")):
        return
    for i in equip:
    ...
    info = {"name": jd["charName"],
            ...
            "status": status,
            "sellTime": expireTime if status == 1 else 0,
            "publicityTime": expireTime if status == 0 else 0,
            "url": url,
            "maxHp": jd["maxHp"],
            "maxMp": jd["maxMp"],
    ...
    if data is not None:
        ...
        print(cursor.execute(updateSql, updateInfo))
    else:
        print(cursor.execute(insertSql, info))
